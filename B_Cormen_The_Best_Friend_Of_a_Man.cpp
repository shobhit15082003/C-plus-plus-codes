...
void solve()
{
    ...
    for(ll i=0;i<n-2;i++)
    {
        ...
        if(pehla<k&&dusra<k)
        {
            ...
            if(i+3>n)
            {
                 v[i+1]=(diff1>=diff2)?diff1:diff2;
            count+=v[i+1];
            }
            else
            {
                ll bo=(v[i]>=v[i+2])?v[i]:v[i+2];
                v[i]=bo;
                v[i+2]=bo;
                v[i+1]=k-bo;
         ...
         else if(dusra>=k)//sirf pehle mai prblm hai
        {
            ll diff=k-v[i+1];
            v[i]=diff;
         //   cout<<"diff = "<<diff<<endl;
    ...
    ll mc=v[v.size()-2]+v[v.size()-1];
    if(mc<k)
    {
        v[v.size()-1]=k-v[v.size()-2];
        count+= v[v.size()-1];
    }
    count=0;
    for(ll i=0;i<n;i++)
