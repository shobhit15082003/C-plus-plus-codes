...
void solve()
{
    ...
    for(ll i=0;i<n;i++)
    {
       ...
       if(find(all(f),input)==f.end())
       {
        ...
        {
            ll index=find(all(f),input)-f.begin();
            ll volue=g[index];
            take=mini(volue,take);
            g[index]=take;
            v[index].ss=take;
        }
   ...
   // cout<<it.ff<<" "<<it.ss<<endl;
    ll time=1,room=0,mino=10e9+1,index=0;
    while(time<mino)
    {
       
        if(room==v[index].ff)
        {
            mino=mini(mino,v[index].ss);
            index++;
        }
        if(time<mino)
        { time++;
        room++;
        //cout<<mino<<" "<<time<<" "<<room<<endl;
        }
      // cout<<mino<<" "<<time<<" "<<room<<endl;
    }
    cout<<time<<endl;
