...
void generateCombinations(string prefix, int length, vector<string> &combinations) {
    if (length == 0) {
    ...
    for (char ch = 'a'; ch <= 'z'; ++ch) {
        generateCombinations(prefix + ch, length - 1, combinations);
    }
}
...
void solve()
{
    ...
    //ll ola=1000;
    ll index=0;
    while(index<combinations.size())
    {
        string now=combinations[index];
        index++;
        // for(ll i='a';i<='z';i++){
        //     now=s;
        //     now+=i;
            if(ss.find(now)==string::npos)
            {
                cout(now);
                return;
            }
