...
void solve()
{
    ll n,d,k,w;
    cin>>n>>k>>d>>w;
    //d->expiry of vaccanise
    // k-> no. of dose
    // w->wait time of patient
    vin(v,n);
    ll count=1,doses=k-1,pivot=0;
    for(ll i=1;i<n;i++)
    {
        if((v[i]-v[pivot]<=(d+w))&&(doses!=0))
        {
            doses--;
            //cout<<"current dose at "<<i<<" count is "<<doses<<endl;
        }
        else
        {
            pivot=i;
    ...
    //     }
    //     while(v[index]-v[i]<=(d+w)&&doses>0&&index<n)
    //     {
    //         index++;
    //         doses--;
    //     }
