...
void solve()
{
    ...
    for(ll i=0;i<n-1;i++)
    {
        ll bfailed=0,cfailed=0,dfailed=0;
        ...
        // b nikal rha hu
        if(i-2>=0){
        ll ac=__gcd(v[i-2],v[i]);
        ll cd=__gcd(v[i],v[i+1]);
        if(ac<=cd)
        {
            b.pb(ac);
            b.pb(cd);
        ...
        continue;
        //c nikal rha hu
        ll ab=__gcd(v[i-2],v[i-1]);
        ll bd=__gcd(v[i-1],v[i+1]);
        if(ab<=bd)
        {
            b.pb(ab);
            b.pb(bd);
